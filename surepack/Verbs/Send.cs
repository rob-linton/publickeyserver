...
class Send 
{
		...
		IProgress<StatusUpdate>? progressOverall = null)
	{
		...
		{
			var surepacks = Storage.ListSurePacks("","outbox");
			int i = 0;
			foreach (var file in surepacks)
			{
				progressOverall?.Report(new StatusUpdate { Index = i, Count = surepacks.Count});
		...
		IProgress<StatusUpdate>? progressFile = null, IProgress<StatusUpdate>? progressOverall = null)
	{
		try
		{
			...
			if (move)
			{
				...
				string sentFolder = Storage.GetSurePackDirectorySent(fromAlias, "");
