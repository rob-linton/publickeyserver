...
public class ReceiveOptions : Options
{
	...
	[Option('f', "force", Default = false, HelpText = "Download all surepacks without prompting")]
	public required bool Force { get; set; } = false;

	[Option('s', "seconds", Default = 0, HelpText = "Check for surepacks every x seconds")]
		...
		IProgress<StatusUpdate>? progressOverall = null)
	{
		...
		try
		{
			...
			while (true)
			{
				...
				// get a list of surepacks from the server
				...
				// sound the bell if there are surepacks waiting
				if (opts.Interval == 0 || receiveCount > 0)
					Misc.LogLine($"\n" + receiveCount + " surepack(s) waiting of " + Misc.FormatBytes(receiveSize) + " total size\n");
				...
				{
					if (opts.Interval == 0)
						Misc.LogLine($"Downloading all surepacks without prompting");
				}
				else
				{
					Misc.LogLine($"Downloading surepacks...");
				}
				...
				// if no surepacks then exit
				if (receiveCount == 0 && opts.Interval == 0)
				{
					Misc.LogLine($"No surepacks waiting\n");
					return 0;
				}
				...
				else
				{
					...
					Misc.LogLine($"\nWhich surepack do you want to receive? ([1-{receiveCount}] or [a] for all, or [q] to quit)");
				...
				foreach (ListFile file in selectedFiles.Files)
				{
					...
					string destFilename = Storage.GetSurePackDirectoryInbox(alias!, filename);

					// now move the surepack to the destination filename
